...
void UI::renderSettingsWindow() {
	...
	if (ImGui::CollapsingHeader("Cache")) {
		// how to pass in actual value when first opens, but then have temp values, then update actual when saved is clicked
		static unsigned int capacity = 20;
		static unsigned int forwardPreload = 5;
		static unsigned int backwardPreload = 2;
		...
		ImGui::InputScalar("##cache capacity", ImGuiDataType_U32, &capacity, &step, nullptr, "%d", ImGuiInputTextFlags_None);
		...
		ImGui::TableSetColumnIndex(1);
		ImGui::SetNextItemWidth(-1.0f);
		ImGui::InputScalar("##cache_forward_preload", ImGuiDataType_U32, &capacity, &step, nullptr, "%d", ImGuiInputTextFlags_None);
		...
		ImGui::TableSetColumnIndex(1);
		ImGui::SetNextItemWidth(-1.0f);
		ImGui::InputScalar("##cache_backward_preload", ImGuiDataType_U32, &capacity, &step, nullptr, "%d", ImGuiInputTextFlags_None);
